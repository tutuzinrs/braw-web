...
const Ranking = () => {
    ...
    const fetchRanking = async () => {
      try {
        const response = await axios.get(
          "https://brawlhalla-championship-backend.onrender.com/users"
        );
  ...
  useEffect(() => {
    if (ranking.length > 0) {
      const maxVictories = Math.max(
        ...ranking.map((player) => player.victorys)
      );
            ...
            <div key={player.id} style={styles.player}>
              ...
              <div style={styles.details}>
                ...
                <div
                  style={{
                    ...styles.animationBar,
                    height: `${victoryBarHeights[player.id] || 0}rem`,
                  }}
                ></div>
              </div>
            </div>
